 ...
 const navigate = useNavigate();
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const res = await axios.get("http://psotsomnapi.runasp.net/api/Post/all"); 
        setPosts(res.data);
      } catch (error) {
      ...
      <div className="card h-100 shadow-sm border-0">
        ...
        <div className="card-body d-flex flex-column justify-content-between">
          ...
          {post.userId?.toString() === currentUserId?.toString() && (
            ...
            <button className="btn px-4 bedit"> <Link to={`/edit/${post.id}`} className="btn bedit">Edit</Link></button> 
              <button className="btn px-4 bdelete" onClick={() => handleDelete(post.id)}>Delete</button>
            </div>
          )}
